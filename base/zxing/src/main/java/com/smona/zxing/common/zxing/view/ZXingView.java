...
import com.smona.zxing.common.zxing.util.BarcodeType;
import com.smona.zxing.common.zxing.core.ScanResult;
import com.smona.zxing.common.zxing.core.QRCodeDecoder;
...
public class ZXingView extends QRCodeView {
    ...
    private int SCALE_FACTOR = 150;

    @Override
    public ScanResult processData(byte[] data, int dataWidth, int dataHeight, boolean isRetry) {
        Result rawResult = null;
        Rect scanBoxAreaRect = null;

        try {
            PlanarYUVLuminanceSource source;
            scanBoxAreaRect = mScanBoxView.getScanBoxAreaRect(dataHeight);
            if (scanBoxAreaRect != null) {
                int left = scanBoxAreaRect.left - SCALE_FACTOR;
                int top = scanBoxAreaRect.top - SCALE_FACTOR;
                int width = scanBoxAreaRect.width() + SCALE_FACTOR;
                int height = scanBoxAreaRect.height() + SCALE_FACTOR;
                if (width > dataWidth) width = dataWidth;
                if (height > dataHeight) height = dataHeight;
                if (left <= 0) left = 0;
                if (top <= 0) top = 0;
                source = new PlanarYUVLuminanceSource(data, dataWidth, dataHeight, left, top, width,
                        height, false);
            } else {
                source = new PlanarYUVLuminanceSource(data, dataWidth, dataHeight, 0, 0, dataWidth, dataHeight, false);
            }
